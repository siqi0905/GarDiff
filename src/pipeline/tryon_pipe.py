...
class StableDiffusionTryOnePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        masked_image = image*(1-mask)+ warped_cloth*mask

        # 4b. process warped cloth
        if cloth_input_type == 'warped':
            cloth_latents = self.vae.encode(warped_cloth).latent_dist.sample(generator=generator)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat(
                    [latent_model_input, masked_image_latents, ], dim=1)
