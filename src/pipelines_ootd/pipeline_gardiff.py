...
def image_Splicing(img_1, img_2, flag='y'):
    size1, size2 = img_1.size, img_2.size # w, h
    if flag == 'x':
...
def max33(x):
    x = Image.fromarray(x)
    x = x.filter(ImageFilter.MaxFilter(1))
    return np.array(x)
...
class tryonpipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        warp_cloth: PipelineImageInput = None,
        ...
        **kwargs,
        ...
        warp_cloth = self.image_processor.preprocess(warp_cloth)
        
        mask_a = np.array(mask)
        mask = np.ones((512, 384), dtype=np.float32)
        for y in range(mask_a.shape[0]):
            for x in range(mask_a.shape[1]):
                # print(source_image[y, x])
                if np.all(mask_a[y, x] == [255,255,255]):  
                    mask[y, x] =1.0
        ...
        cloth_latents = self.vae.encode(warp_cloth).latent_dist.sample(generator=generator)
        cloth_latents = self.vae.config.scaling_factor * cloth_latents
        
        warp_cloth_latents = self.prepare_garm_latents(
            warp_cloth,
            batch_size,
            num_images_per_prompt,
            prompt_embeds.dtype,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_vton_model_input = torch.cat([scaled_latent_model_input, vton_latents,warp_cloth_latents], dim=1)
                ...
                if i<18:
                    ...
                    latents = latents * (1-mask_latents) + cloth_latents * (mask_latents)
    ...
    def prepare_vton_latents(
        ...
        if image.shape[1] == 4:
            ...
            if isinstance(generator, list):
                ...
                # mesh_latents = self.vae.encode(mesh).latent_dist.mode()
        ...
        #     depth, size=(image_latents.size(-2), image_latents.size(-1))
        # )
        ...
        if batch_size > image_latents.shape[0] and batch_size % image_latents.shape[0] == 0:
            ...
            image_ori_latents = torch.cat([image_ori_latents], dim=0)
            # mesh_latents = torch.cat([mesh_latents], dim=0)
            # depth = torch.cat([depth], dim=0)

        if do_classifier_free_guidance:
            # uncond_image_latents = torch.zeros_like(image_latents)
            image_latents = torch.cat([image_latents] * 2, dim=0)
            # mesh_latents = torch.cat([mesh_latents]*2, dim=0)
            # depth = torch.cat([depth]*2, dim=0)
